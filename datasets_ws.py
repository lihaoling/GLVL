...
def dataLoader(config, dataset='syn', warp_input=False, train=True, val=True):
    ...
    # else:
    mod = importlib.import_module('{}.{}'.format('datasets', dataset))
...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        if dataset_name == "ICRA2022":
            ...
            with open(self.dataset_folder + '/gt_matches.csv', 'rt') as csvfile:
                reader = csv.DictReader(csvfile)
                column = [row['ref_ind'] for row in reader]
                column = list(map(int, column))
            self.soft_positives_per_query = np.array(column)
            ...
            database_folder = join(self.dataset_folder, "reference_images")
            queries_folder = join(self.dataset_folder, "query_images")
            if not os.path.exists(database_folder):
                raise FileNotFoundError(f"Folder {database_folder} does not exist")
            if not os.path.exists(queries_folder):
        ...
        elif dataset_name == "TianJin":
            #### Read paths and UTM coordinates for all images.
            self.dataset_folder = datasets_folder
            if not os.path.exists(self.dataset_folder):
                raise FileNotFoundError(f"Folder {self.dataset_folder} does not exist")
            database_folder = join(self.dataset_folder, "patch")
            queries_folder = join(self.dataset_folder, "B5/101MSDCF")
            if not os.path.exists(database_folder):
                raise FileNotFoundError(f"Folder {database_folder} does not exist")
            if not os.path.exists(queries_folder):
                raise FileNotFoundError(f"Folder {queries_folder} does not exist")
            ...
            self.database_paths = sorted(glob(join(database_folder, "**", "*.png"), recursive=True))
            self.queries_paths = sorted(glob(join(queries_folder, "**", "*.JPG"), recursive=True))
        elif dataset_name == "village" or dataset_name == "gravel_pit":
            #### Read paths and UTM coordinates for all images.
            self.dataset_folder = datasets_folder
            if not os.path.exists(self.dataset_folder):
                raise FileNotFoundError(f"Folder {self.dataset_folder} does not exist")
            database_folder = join(self.dataset_folder, "patch")
            queries_folder = join(self.dataset_folder, "frames")
            if not os.path.exists(database_folder):
                raise FileNotFoundError(f"Folder {database_folder} does not exist")
            if not os.path.exists(queries_folder):
                raise FileNotFoundError(f"Folder {queries_folder} does not exist")
            ...
            self.dataset_folder =  datasets_folder
            self.subdataset_folder = os.path.join(datasets_folder, 'Manchester_'+dataset_name)
            self.frame_folder = os.path.join(self.subdataset_folder, 'frames')
            self.patch_folder = os.path.join(self.subdataset_folder, 'patch')


            if not os.path.exists(self.dataset_folder):
                raise FileNotFoundError(f"Folder {self.dataset_folder} does not exist")
            database_folder = join(self.subdataset_folder, "patch")
            queries_folder = join(self.subdataset_folder, "frames")
            if not os.path.exists(database_folder):
                raise FileNotFoundError(f"Folder {database_folder} does not exist")
            if not os.path.exists(queries_folder):
                raise FileNotFoundError(f"Folder {queries_folder} does not exist")
            ...
            self.frames = sorted(os.listdir(queries_folder))



        # else:
        #     database_folder = join(self.dataset_folder, "patch/refer")
        #     queries_folder = join(self.dataset_folder, "test/query/")
        #     self.database_paths = sorted(glob(join(database_folder, "**", "*.jpg"), recursive=True))
        #     self.queries_paths = sorted(glob(join(queries_folder, "**", "*.jpg"), recursive=True))
        #
    ...
    def __getitem__(self, index):
        ...
        if self.test_method == "hard_resize":
            ...
            img = T.functional.resize(img, self.resize, antialias=None)
        else:
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        if dataset_name == "ICRA2022" or dataset_name == "AID":
            self.hard_positives_per_query = self.soft_positives_per_query
    ...
    def compute_cache(args, model, subset_ds, cache_shape):
        ...
        with torch.no_grad():
            for images, indexes in tqdm(subset_dl, ncols=100):
                ...
                features = model(images, 'retrieval') #train_jointly
    ...
    def get_best_positive_index(self, args, query_index, cache, query_features):
        if args.dataset_name == 'ICRA2022'or args.dataset_name == 'AID':
